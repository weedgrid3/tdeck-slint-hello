...
# First try to find locally available package
find_package(Slint ${Slint_FIND_VERSION} QUIET CONFIG)
if (TARGET Slint::Slint)
    # Already found, don't download again
    return()
endif()
...
if (NOT SLINT_TARGET_ARCHITECTURE)
    if(WIN32)
        ...
        # Try multiple ways to detect ESP-IDF target
        set(TARGET_ARCH "")
        
        # 1. Check if IDF_TARGET is set as cmake variable (set by idf.py)
        if (IDF_TARGET)
            set(TARGET_ARCH ${IDF_TARGET})
        endif()
        
        # 2. Check environment variable
        if (NOT TARGET_ARCH AND DEFINED ENV{IDF_TARGET})
            set(TARGET_ARCH $ENV{IDF_TARGET})
        endif()
        
        # 3. Check CMAKE_SYSTEM_NAME and CMAKE_SYSTEM_PROCESSOR for ESP-IDF
        if (NOT TARGET_ARCH AND CMAKE_SYSTEM_NAME STREQUAL "Generic" AND CMAKE_SYSTEM_PROCESSOR MATCHES "esp32")
            # Try to extract from processor name
            string(REGEX MATCH "(esp32[a-z0-9]*)" MATCHED_ARCH ${CMAKE_SYSTEM_PROCESSOR})
            if (MATCHED_ARCH)
                set(TARGET_ARCH ${MATCHED_ARCH})
            endif()
        endif()
        
        # 4. Check if we can detect from toolchain file or compiler
        if (NOT TARGET_ARCH)
            if (CMAKE_C_COMPILER MATCHES "xtensa")
                # Could be esp32 or esp32s2/s3
                set(TARGET_ARCH "esp32s3")
            elseif (CMAKE_C_COMPILER MATCHES "riscv32")
                set(TARGET_ARCH "esp32c3")
            endif()
        endif()
        
        # Map to Slint architecture names
        if (TARGET_ARCH)
            if (TARGET_ARCH STREQUAL "esp32s3" OR TARGET_ARCH STREQUAL "esp32s2")
                set(SLINT_TARGET_ARCHITECTURE "xtensa-${TARGET_ARCH}-none-elf")
            elseif (TARGET_ARCH STREQUAL "esp32c3" OR TARGET_ARCH STREQUAL "esp32c2" OR TARGET_ARCH STREQUAL "esp32c6" OR TARGET_ARCH STREQUAL "esp32h2")
                set(SLINT_TARGET_ARCHITECTURE "riscv32imc-esp-espidf")
            elseif (TARGET_ARCH STREQUAL "esp32p4")
                set(SLINT_TARGET_ARCHITECTURE "riscv32imafc-esp-espidf")
            else()
                message(WARNING "Unknown IDF_TARGET: ${TARGET_ARCH}, using default")
                set(SLINT_TARGET_ARCHITECTURE "${CMAKE_SYSTEM_NAME}-${CMAKE_SYSTEM_PROCESSOR}")
            endif()
        else()
            # Fallback - try to detect from compiler
            if (CMAKE_C_COMPILER MATCHES "xtensa")
                message(WARNING "Could not detect exact ESP32 variant, assuming esp32s3")
                set(SLINT_TARGET_ARCHITECTURE "xtensa-esp32s3-none-elf")
            elseif (CMAKE_C_COMPILER MATCHES "riscv32")
                message(WARNING "Could not detect exact ESP32 variant, assuming esp32c3")
                set(SLINT_TARGET_ARCHITECTURE "riscv32imc-esp-espidf")
            else()
                set(SLINT_TARGET_ARCHITECTURE "${CMAKE_SYSTEM_NAME}-${CMAKE_SYSTEM_PROCESSOR}")
            endif()
...
# Now find the package we just downloaded - this will define the targets
find_package(Slint ${Slint_FIND_VERSION} CONFIG REQUIRED)